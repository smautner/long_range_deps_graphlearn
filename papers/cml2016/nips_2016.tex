...
\begin{figure}[ht]
      ...
      Mid: RNA encoding with nucleotide level resolution.  Right:  RNA encoding
      with structural element resolution, with symbols: H)airpin, M)ultiloop,
      S)tem, D)angling end.}
      \label{allcips}
\end{figure}
...
sequences as the training set increases. The horizontal line indicates
...
To make it comparable to our method, we used MUSCLE and RNAalifold to
train a new Infernal model and report the
performance of its generated sequences (dashed).  
We note
