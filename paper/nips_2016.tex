...
In Fig. \ref{allcips} (left) we can see a production step applied to a
molecular graph $G$. The core $C_{R}^v(G)$ (dark) is determined by vertices at
maximal distance $R$ ($R=0$ in this case) from a chosen root vertex $v$. In a
lighter shade the interface $I_{R,T}^v(G)$ with \emph{thickness} $T=2$. This
is the graph induced by the vertices at maximal distance $R+1+T=2$ and minimal
distance $R+1=1$ from $v$. A new core-interface pair (CIP) is available with
matching interface and a different core: the substitution can take place and
yields th replacement of a carbon into a nitrogen atom.

\cite{costa16} showed how the production rules (i.e. the CIPs) can be efficiently extracted from a set of graphs that is representative of the desired concept.
...
a) resolution issues and b) a viability problem. Replacing only few nodes at
each iteration can be inefficient when instances consist of several hundreds
of nodes. It is known that, in the case of RNA, a more effective description
is obtained considering larger structural components such as {\em stems}
(stretches of consecutive paired nucleotides) and {\em loops} (stretches of
consecutive unpaired nucleotides). As for b), it is known that the function of
RNA polymers depends on their global structure (i.e. the set of pairs of
interacting nucleotides), which can significantly change when even a single
nucleotide is altered. To deal with these issues we propose two contributions
to Costa's approach: a) user defined grammar coarsening and b) user defined
...
the two vertices that it previously joined. In addition we allow the less
restrictive notion of {\em vertex contraction}: which removes the restriction
that the contraction must occur over vertices sharing an incident edge. Both
procedures are implemented defining a node attribute $cid \in \IN$ called {\em
contraction ID} and contracting all vertices that share the same $cid$ value.
The contraction operation transforms graphs to their coarser versions $G
\mapsto G'$. Cores and interfaces are now defined using the coarse version. We
consider the original CIP definition on the coarse graph: to define the core
we identify all vertices of the original graph that have been contracted in
the coarse core
...
\fxwarning{style of the underset stuff? }
