...
the proposal ($g$) and 2) the acceptance-rejection ($A$) probability
distributions. The samples are obtained by an iterative procedure that
modifies instances using $g$ which are then filtered by $A$. The key element
for an efficient MH design consists in shaping the proposal distribution  in
such a way that the generated elements will not be rejected too often. To do
so Costa suggests to use a grammar and infer its rules from the
available data using grammatical inference techniques upgraded to structured
domains (i.e. domains where instances are naturally represented as labeled
graphs).
...
\begin{figure}[ht]
      ...
      respectively. Left: Molecular graph with highlighted CIP; alternative matching interface and.
      Mid: RNA encoding with nucleotide level resolution.  Right:  RNA encoding with structural elements encoding: H)airpin, M)ultiloop, S)tem, D)anglin end.}
      \label{allcips}
\end{figure}


\textbf{Extension of core and interface notion.}


2 problems for RNA: 1)  we would like larger blocks, 2) refolding.
