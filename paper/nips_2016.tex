...
\cite{costa16} the problem of generating elements in a structured domain was
...
Approaches like ''rejection sampling'' where samples are generated without
...
valid samples. Instead \cite{costa16} proposes to use a Metropolis Hastings
(MH) Markov Chain Monte Carlo (MCMC) method, where the problem of sampling is
reduced to the easier task of 'simulation', provided that one can build an
ergodic Markov chain with the desired distribution as its equilibrium
distribution. They define a context sensitive graph grammar to inform the MH
proposal distribution and use a probability density estimator based on a graph
kernel approach to define the MH acceptance procedure. The authors acknowledge
that a grammar approach  is not sufficient to solve the constructive machine
learning problem. In fact, the number of proposed graphs grows exponentially
with the number of production rules in the grammar. In order to select only
the most promising candidates one needs to equip the grammar with a
probabilistic notion. However any procedure to estimate the probabilities of
such locally context-sensitive grammar needs to trade off the number of
contextual rules with the expressiveness of the grammar. The larger the
context, the (exponentially) larger the number of possible cases that need to
be observed for a reliable estimate, which directly implies an exponentially
large number of training examples. For this reason global or just long range
constraints cannot be effectively modeled which in turn implies a language
that can express non- viable instances. On the other hand, a probability
density estimator based on a graph kernel approach cannot be used in a
generative mode, i.e. it cannot be used to actually construct graphs. The
solution proposed in \cite{costa16} instead employs both approaches to deal
separately with local and global constraints: a locally context-sensitive
graph grammar is used for the local constraints and a regularized statistical
...
Unfortunately in some applications, such as those of chemo- and bio-
informatics, most domain instances exhibit complex dependencies between their
